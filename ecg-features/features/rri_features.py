...
class RRIFeatures:
    ...
    def __init__(
        ...
        # Update feature dictionary to add rpeaks values
        self.rri_features['rpeaks_x_enzo'] = self.rpeaks
    ...
    def calculate_rri_nonlinear_features(self, rri, diff_rri, diff2_rri):
        ...
        if len(rri) > 1:
            rri_nonlinear_features['rri_entropy'] = self.safe_check(
                ent.sample_entropy(rri, sample_length=2, tolerance=0.1 * np.std(rri))[0]
            )
        ...
        if len(diff_rri) > 1:
            rri_nonlinear_features['diff_rri_entropy'] = self.safe_check(
                ent.sample_entropy(diff_rri, sample_length=2, tolerance=0.1 * np.std(diff_rri))[0]
            )
        ...
        if len(diff2_rri) > 1:
            rri_nonlinear_features['diff2_rri_entropy'] = self.safe_check(
                ent.sample_entropy(diff2_rri, sample_length=2, tolerance=0.1 * np.std(diff2_rri))[0]
            )
